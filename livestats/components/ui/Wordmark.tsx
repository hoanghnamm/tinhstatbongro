 ...
 *
 * `hooprec` was written inline in the lobby for as long as the lobby was the
 ...
 * angle, or with `rec` a half-step off the accent — which is precisely what
...
/**
 * `hoop` in ink, `rec` in accent, leaning as ONE mark.
 *
 ...
 *
 * IT IS TWO NODES AND NOT A SPAN, because `rec` is painted with `glowInk` —
 ...
 * top edge. The skew wraps BOTH, or `rec` leans at its own angle.
 *
...
export function Wordmark({ size }: { size?: number }) {
  ...
  const face = {
    ...
    // LOWERCASE now: `p` descends, and the caps-height box that
    // fitted `HOOPREC` cuts the tail off `hooprec`.
    lineHeight: fs * 1.3,
